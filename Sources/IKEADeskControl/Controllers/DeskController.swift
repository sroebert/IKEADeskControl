...
actor DeskController {
    ...
    init(peripheralUUID: UUID?) async {
        controller = await PeripheralController(
            serviceUUID: Services.control,
    ...
    private func connectAndDiscover(_ peripheral: CBPeripheral) async throws {
        ...
        let deskState = try deskState(for: positionCharacteristic)
    ...
    private func deskState(for positionCharacteristic: CBCharacteristic) throws -> DeskState {
        guard let value = positionCharacteristic.value else {
    ...
    private var deskState: DeskState {
        get throws {
            guard let positionCharacteristic = connection?.positionCharacteristic else {
                throw ControllerError.notConnected
            }
            return try deskState(for: positionCharacteristic)
        }
    }
